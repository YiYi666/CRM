							...
							<tr>
								...
								<td colspan="3">
									<select name="custId" style="WIDTH: 180px">
												<option value="${linkman.customer.custId}"  selected="selected" >${linkman.customer.custId}</option>
										<c:forEach items="${custIds }" var="cId">
											<option value="${cId}" >${cId}</option>
										</c:forEach>
									</select>
									<%--<input type="text" name="custId" style="WIDTH: 180px" value="${linkman.customer.custId}" />--%>
									<%--	<input list="cid" name="custId" style="WIDTH: 180px"/>
										<datalist id="cid">

											<c:forEach items="${custIds }" var="cId">
												<option value="${cId}" <c:if test="${linkman.customer.custId== cId} ">selected</c:if> >${cId}</option>
											</c:forEach>
										</datalist>--%>
								</td>
							</tr>
